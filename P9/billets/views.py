...
class BilletCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        form.instance.user = self.request.user  # associe le billet à l’utilisateur connecté
...
class BilletUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def get_queryset(self):
        return Billet.objects.filter(user=self.request.user)  # permet uniquement à l’auteur de modifier
...
class BilletDeleteView(LoginRequiredMixin, DeleteView):
    ...
    def get_queryset(self):
        return Billet.objects.filter(user=self.request.user)  # permet uniquement à l’auteur de supprimer

@login_required
def flux(request):
    billets = Billet.objects.all().order_by('-time_created')  # trie les billets par date de création décroissante
...
class CommentaireCreateView(LoginRequiredMixin, CreateView):
    ...
    def get_success_url(self):
        return reverse_lazy('billets:flux')  # ou vers une page billet précise si vous préférez
