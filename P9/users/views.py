        ...
        if form.is_valid():
            ...
            try:
                ...
                if user_to_follow == request.user:
                    ...
                    existence = UserFollows.objects.filter(user=request.user, followed_user=user_to_follow).exists()
                    if existence:
...
class UserFollowsDeleteView(LoginRequiredMixin, DeleteView):
    ...
    def get_queryset(self):
        # Ne permettre à l’utilisateur connecté de supprimer que ses propres suivis
        return UserFollows.objects.filter(user=self.request.user)